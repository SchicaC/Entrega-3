...
class MiembrosView:
    ...
    def setup_ui(self):
        ...
        self.titulo = tk.Label(self.left_frame, text="👥 GESTIÓN DE MIEMBROS",
    ...
    def create_form_fields(self):
        # ID y Buscar
        tk.Label(self.form_frame, text="ID Miembro:", font=("Arial", 10)).grid(
            row=0, column=0, sticky="w", padx=(0, 5), pady=5)
        self.IDMiembros = tk.Entry(self.form_frame, width=20, font=("Arial", 10), relief="solid", bd=1)
        self.IDMiembros.grid(row=0, column=1, sticky="w", pady=5)
        ...
        # Primera columna
        tk.Label(self.form_frame, text="Número Socio:", font=("Arial", 10)).grid(
            row=1, column=0, sticky="w", padx=(0, 5), pady=5)
        self.NumeroSocio = tk.Entry(self.form_frame, width=20, font=("Arial", 10), relief="solid", bd=1)
        self.NumeroSocio.grid(row=1, column=1, sticky="w", pady=5)
        ...
        tk.Label(self.form_frame, text="Dirección:", font=("Arial", 10)).grid(
            row=6, column=0, sticky="w", padx=(0, 5), pady=5)
        self.Direccion = tk.Entry(self.form_frame, width=20, font=("Arial", 10), relief="solid", bd=1)
        self.Direccion.grid(row=6, column=1, sticky="w", pady=5)

        tk.Label(self.form_frame, text="Teléfono:", font=("Arial", 10)).grid(
            row=7, column=0, sticky="w", padx=(0, 5), pady=5)
        self.Telefono = tk.Entry(self.form_frame, width=20, font=("Arial", 10), relief="solid", bd=1)
        self.Telefono.grid(row=7, column=1, sticky="w", pady=5)

        tk.Label(self.form_frame, text="Correo Electrónico:", font=("Arial", 10)).grid(
            row=8, column=0, sticky="w", padx=(0, 5), pady=5)
        self.CorreoElectronico = tk.Entry(self.form_frame, width=20, font=("Arial", 10), relief="solid", bd=1)
        self.CorreoElectronico.grid(row=8, column=1, sticky="w", pady=5)

        tk.Label(self.form_frame, text="Fecha Ingreso:", font=("Arial", 10)).grid(
            row=9, column=0, sticky="w", padx=(0, 5), pady=5)
        self.FechaIngreso = DateEntry(self.form_frame, width=18, date_pattern='yyyy-mm-dd')
        self.FechaIngreso.grid(row=9, column=1, sticky="w", pady=5)
        ...
        self.Categoria.grid(row=0, column=4, sticky="w", pady=5)

        tk.Label(self.form_frame, text="Modalidad Deportiva:", font=("Arial", 10)).grid(
            row=1, column=3, sticky="w", padx=(50, 5), pady=5)
        self.ModalidadDeportiva = tk.Entry(self.form_frame, width=25, font=("Arial", 10), relief="solid", bd=1)
        self.ModalidadDeportiva.grid(row=1, column=4, sticky="w", pady=5)

        tk.Label(self.form_frame, text="Nivel:", font=("Arial", 10)).grid(
            row=2, column=3, sticky="w", padx=(50, 5), pady=5)
        self.Nivel = tk.Entry(self.form_frame, width=25, font=("Arial", 10), relief="solid", bd=1)
        self.Nivel.grid(row=2, column=4, sticky="w", pady=5)

        tk.Label(self.form_frame, text="Historial Médico:", font=("Arial", 10)).grid(
            row=3, column=3, sticky="w", padx=(50, 5), pady=5)
        self.HistorialMedico = tk.Entry(self.form_frame, width=25, font=("Arial", 10), relief="solid", bd=1)
        self.HistorialMedico.grid(row=3, column=4, sticky="w", pady=5)

        tk.Label(self.form_frame, text="Cuota Asignada:", font=("Arial", 10)).grid(
            row=4, column=3, sticky="w", padx=(50, 5), pady=5)
        self.CuotaAsignada = tk.Entry(self.form_frame, width=25, font=("Arial", 10), relief="solid", bd=1)
        self.CuotaAsignada.grid(row=4, column=4, sticky="w", pady=5)

        tk.Label(self.form_frame, text="Estado:", font=("Arial", 10)).grid(
            row=5, column=3, sticky="w", padx=(50, 5), pady=5)
        self.Estado = ttk.Combobox(self.form_frame, width=23, values=["Activo", "Inactivo", "Suspendido"])
        self.Estado.grid(row=5, column=4, sticky="w", pady=5)
        ...
        tk.Label(self.form_frame, text="Fecha Pago:", font=("Arial", 10)).grid(
            row=7, column=3, sticky="w", padx=(50, 5), pady=5)
        self.FechaPago = DateEntry(self.form_frame, width=23, date_pattern='yyyy-mm-dd')
        self.FechaPago.grid(row=7, column=4, sticky="w", pady=5)

        tk.Label(self.form_frame, text="Fecha Vencimiento:", font=("Arial", 10)).grid(
            row=8, column=3, sticky="w", padx=(50, 5), pady=5)
        self.FechaVencimiento = DateEntry(self.form_frame, width=23, date_pattern='yyyy-mm-dd')
        self.FechaVencimiento.grid(row=8, column=4, sticky="w", pady=5)

        tk.Label(self.form_frame, text="Estado Pago:", font=("Arial", 10)).grid(
            row=9, column=3, sticky="w", padx=(50, 5), pady=5)
        self.EstadoPago = ttk.Combobox(self.form_frame, width=23, values=["Pagado", "Pendiente", "Vencido"])
        self.EstadoPago.grid(row=9, column=4, sticky="w", pady=5)
        self.EstadoPago.set("Pendiente")

    def create_image_section(self):
        self.image_frame = tk.LabelFrame(self.left_frame, text="🖼️ Foto del Miembro", font=("Arial", 10))
    ...
    def create_toolbar(self):
        ...
        tk.Label(self.toolbar, text="LISTA DE MIEMBROS", font=("Arial", 14, "bold")).pack(side="left", padx=10)
    ...
    def create_treeview(self):
        self.treeview = ttk.Treeview(self.right_frame,
                                     columns=('ID', 'NumeroSocio', 'Nombres', 'Apellidos', 'DocumentoIdentidad',
                                              'FechaNacimiento', 'Direccion', 'Telefono', 'CorreoElectronico',
                                              'FechaIngreso',
                                              'Categoria', 'ModalidadDeportiva', 'Nivel', 'HistorialMedico',
                                              'CuotaAsignada',
                                              'Estado', 'IDEquipos', 'FechaPago', 'FechaVencimiento', 'EstadoPago'),
                                     show='headings', height=20)

        columns_config = [
            ('ID', 50), ('NumeroSocio', 80), ('Nombres', 100), ('Apellidos', 100),
            ('DocumentoIdentidad', 100), ('FechaNacimiento', 100), ('Direccion', 100),
            ('Telefono', 80), ('CorreoElectronico', 120), ('FechaIngreso', 100),
            ('Categoria', 80), ('ModalidadDeportiva', 120), ('Nivel', 80),
            ('HistorialMedico', 120), ('CuotaAsignada', 80), ('Estado', 80),
            ('IDEquipos', 80), ('FechaPago', 100), ('FechaVencimiento', 100),
            ('EstadoPago', 80)
        ]
